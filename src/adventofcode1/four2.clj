...
;; (s/def ::name (s/spec string? :gen #(gen'/string-from-regex #"([a-c]{3}-)*[0-9]+\[[a-z]+\]"))) ; => YES
;;(s/def ::name (s/with-gen string? #{"aaaaa-bbb-z-y-x-123[abxyz]" "aaaaa-bbb-z-y-x-123[error]"})) :=> no
;;(s/def ::name (s/with-gen string? (s/spec #{"aaaaa-bbb-z-y-x-123[abxyz]" "aaaaa-bbb-z-y-x-123[error]"}))) ;=> no
;;(s/def ::name #{"aaaaa-bbb-z-y-x-123[abxyz]" "aaaaa-bbb-z-y-x-123[error]"}) ;=> works
;; (s/def ::name (s/with-gen string? (fn [] (s/gen #{"aaaaa-bbb-z-y-x-123[abxyz]" "aaaaa-bbb-z-y-x-123[error]"})))) ;=> yes
;; (s/def ::name (s/with-gen string? #(s/gen #{"aaaaa-bbb-z-y-x-123[abxyz]" "aaaaa-bbb-z-y-x-123[error]"}))) ;=> yes
...
(s/fdef sum-sectorid-of-valid-room-names
        :args (s/cat :names (s/coll-of ::full-name))
        :ret  integer?)
...
(s/fdef find-northpole
        ...
        :args (s/cat :names (s/coll-of ::full-name :min-count 1))
